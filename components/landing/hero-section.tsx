...
import { Search, Users, Flame, Music, Mic2, Laptop, Sparkles, Ticket } from "lucide-react";
...
export function HeroSection() {
  ...
  return (
    <section className="hero-section relative w-full px-4 pt-6 md:px-10 md:pt-5 font-switzer">
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        className="hero-container relative h-160 w-full overflow-hidden rounded-[3.5rem] shadow-sm"
      >
          ...
          <div className="w-full max-w-4xl">
            <div className="hero-search flex flex-col gap-0 rounded-[2.5rem] p-2 md:flex-row md:items-center md:rounded-full">
              ...
              <div className="hero-input-group flex-1 px-8 py-3 text-left group transition-colors rounded-l-full md:border-r">
                ...
                <span className="hero-input-text w-full bg-transparent text-sm font-semibold">
                  Visakhapatnam
                </span>
              </div>
              ...
              <div className="hero-input-group flex-1 px-8 py-3 text-left group transition-colors md:border-r">
                ...
                <Select>
                  <SelectTrigger className="hero-input-text w-full bg-transparent text-sm font-semibold outline-none cursor-pointer border-none shadow-none focus:ring-0">
                    <SelectValue placeholder="Any Activity" />
                  </SelectTrigger>
                  <SelectContent className="bg-popover border border-input rounded-md shadow-md">
                    {categories.map((cat) => (
